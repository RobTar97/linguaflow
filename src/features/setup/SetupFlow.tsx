...
export default function SetupFlow() {
  ...
  const [level, setLevel] = useState<Level>(
    profile?.goal.level ??
      practiceIntent?.level ??
      workspaceDefaults.goal.level,
  );
  ...
  function setNative(next: LanguageCode) {
    ...
    if (next === targetLanguage) {
      setTargetLanguage(
        languages.find((language) => language !== next) ?? "EN",
      );
    }
  }
  ...
  function setTarget(next: LanguageCode) {
    ...
    if (next === nativeLanguage) {
      setNativeLanguage(
        languages.find((language) => language !== next) ?? "PL",
      );
    }
  }
  ...
  return (
    <main className="setup-shell">
      ...
      <AmicroFadeUp className="setup-card-motion">
